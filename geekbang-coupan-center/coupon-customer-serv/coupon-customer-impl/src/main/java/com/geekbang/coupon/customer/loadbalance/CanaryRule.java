...
public class CanaryRule implements ReactorServiceInstanceLoadBalancer {
    ...
    Response<ServiceInstance> getInstanceResponse(List<ServiceInstance> instances, Request request){
        ...
        if(CollectionUtils.isEmpty(instances)){
            log.warn("No instance avaliable {}", serviceId);
        ...
        if(StringUtils.isBlank(trafficVersion)){
            //过滤掉所有金丝雀测试的节点，则Nacos Metadata中包含流量标记的节点
            ...
            return getRoundRobinIntance(noneCanaryInstances);
        }

        //如果Webclient的header里包含流量标记
        //循环每个Nacos服务节点，过滤出matadata值相同的instance，再使用RoundRobin查找
        ...
        return getRoundRobinIntance(canaryInstances);
    }

    private Response<ServiceInstance> getRoundRobinIntance(List<ServiceInstance> instances){
